...
public class ZooKeeper implements AutoCloseable {
    ...
    public ZooKeeper(
        ...
        //初始化一个客户端的WatchManager
        watchManager = defaultWatchManager();
        //defaultWatcher: 全局的watch
        ...
        cnxn = createConnection(
            ...
            this,
            watchManager,
            getClientCnxnSocket(), //构建一个客户端网络通信的实例  ClientCnxnSocketNIO
            canBeReadOnly);
        cnxn.start();
    }
    ...
    public Stat exists(final String path, Watcher watcher) throws KeeperException, InterruptedException {
        ...
        if (watcher != null) {
            //把watcher包装成ExistsWatchRegistration
        ...
        //请求头
        RequestHeader h = new RequestHeader();
        h.setType(ZooDefs.OpCode.exists); //操作类型
        //请求体
        ExistsRequest request = new ExistsRequest();
        request.setPath(serverPath);
        request.setWatch(watcher != null); //传递是否有注册监听的布尔值
        SetDataResponse response = new SetDataResponse(); //返回值
