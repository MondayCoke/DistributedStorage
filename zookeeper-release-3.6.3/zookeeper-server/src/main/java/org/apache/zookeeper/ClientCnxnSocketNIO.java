...
public class ClientCnxnSocketNIO extends ClientCnxnSocket {
    ...
    void doIO(Queue<Packet> pendingQueue, ClientCnxn cnxn) throws InterruptedException, IOException {
        ...
        if (sockKey.isReadable()) {
            ...
            if (!incomingBuffer.hasRemaining()) {
                ...
                if (incomingBuffer == lenBuffer) {
                    ...
                    //读取服务端的消息
                    //incomingBuffer表示服务端传递过来的 ExistsResponse对象
        ...
        if (sockKey.isWritable()) {
            ...
            if (p != null) {
                ...
                sock.write(p.bb); //已经把packet序列化后写入到socket
    ...
    void registerAndConnect(SocketChannel sock, InetSocketAddress addr) throws IOException {
        ...
        boolean immediateConnect = sock.connect(addr); //和服务端addr地址建立连接
        if (immediateConnect) {
    ...
    void doTransport(
        ...
        selector.select(waitTimeOut); //带有超时时间的阻塞： wakeup()
        Set<SelectionKey> selected;
        synchronized (this) {
            selected = selector.selectedKeys();  //选择就绪的连接
        }
        // Everything below and until we get back to the select is
        // non blocking, so time is effectively a constant. That is
        // Why we just have to do this once, here
        updateNow();
        for (SelectionKey k : selected) { //遍历就绪的socketChannel
