...
public class FastLeaderElection implements Election {
            ...
            public void run() {
                while (!stop) {
                    try {
                        //生产者消费者模型
                        //从阻塞队列获取任务，去执行
                        //一定会有一个地方往该队列添加任务, sendqueue
                        // 在进行投票选举的时候， 当前节点需要把vote发出去的时候。
    ...
    private void sendNotifications() {
        //sid 表是配置的集群中de  server.1.2.3
        for (long sid : self.getCurrentAndNextConfigVoters()) {
            //QuorumMaj： 判断的
            QuorumVerifier qv = self.getQuorumVerifier();
            ToSend notmsg = new ToSend(
                ToSend.mType.notification, //消息类型
                proposedLeader,  //推荐的leader的sid/myid| 第一次对应的是自己
                proposedZxid, //zxid 事务id
                logicalclock.get(),  //逻辑时钟（epoch）
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            ...
            synchronized (this) {
                logicalclock.incrementAndGet(); //逻辑时钟（epoch）
                /**
                 * 第一票是给自己的
                 *
                 * getInitId:  sid(serverid) - myid(当前节点的myid)
                 * getInitLastLoggedZxid: 得到最新的zxid
                 * getPeerEpoch: 当前节点的epoch
                 */
             ...
             */
            //只要当前节点是LOOKING并且没有停止，则一定要选出leader
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                 ...
                 */
                    ...
                    case LOOKING:
                        ...
                        //接收的票据的epoch和当前节点的epoch
                        if (n.electionEpoch > logicalclock.get()) {
                            //把自己的epoch改成收到票据的epoch
                            logicalclock.set(n.electionEpoch);
                            recvset.clear(); //清空本地接受者的数据

                            if (totalOrderPredicate(n.leader, n.zxid, n.peerEpoch, getInitId(), getInitLastLoggedZxid(), getPeerEpoch())) {
                                //对方的票据优先级比我的高，你我要妥协。
                                //意味着如果有下一次投票，那就这个票据发送给集群中的节点
